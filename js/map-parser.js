...
MapParser = {
  ...
  roundPaths: function (arr, scale) {
    ...
    // const size = {width: fakeSeries.maxX, height: fakeSeries.maxY};
    ...
    arr.forEach(function (point) {
      var i, path;
      point.path = path = mapProto.translatePath.call(
        fakeSeries,
        point.path,
        true
      );
      ...
      if (point.name !== "__separator_lines__") {
  ...
  loadSVG: function (url) {
    ...
    function getPathLikeChildren(parent) {
      return Array.prototype.slice
        ...
        .concat(
          Array.prototype.slice.call(parent.getElementsByTagName("polygon"))
        )
        .concat(
          Array.prototype.slice.call(parent.getElementsByTagName("rect"))
        );
    }
    ...
    function handleSVG(xml) {
      ...
      ["defs", "clipPath"].forEach(function (nodeName) {
        ...
        //     });
        // });
        const parents = xml.getElementsByTagName(nodeName);
        for (let i = 0; i < parents.length; i++) {
          const paths = parents[i].getElementsByTagName("path");
      ...
      if (handleGroups) {
        const groups = lastCommonAncestor.getElementsByTagName("g");
        for (let i = 0; i < groups.length; i++) {
          ...
          getPathLikeChildren(g).forEach(function (path) {
            if (!path.skip) {
              originalPath = getPathDefinition(path);
      ...
      const result = data.roundPaths(arr);
      return result.size;
